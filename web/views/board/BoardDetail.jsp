	...
	<% if(loginMember != null) { %>
		...
		<div id="table_conntents">
			<div id="boardCount_div">
				<form action="<%=request.getContextPath() %>" method="get" onsubmit="return ok()">
					<table id="table">
					<tr>
						<td>제목</td>
						<td>
							<input type="text" value="<%=list.get("BoardTitle") %>" name="title">
							<input type="hidden" value="<%= loginMember.getMemberNo() %>" name="memberNo">
						</td>
					</tr>
					<tr>
						<td>작성자</td>
						<td><input type="text" value="<%=list.get("BoardMemberName")%>" name="writer" readonly></td>
					</tr>
					<tr>
						<td>게시판 종류</td>
					...
					<tr>
						<td colspan="2">
							<textarea id="textArea" rows="" cols="" value="<%=list.get("BoardCount") %>" name="content" readonly></textarea>
						</td>
					</tr>
					<tr>
						<td colspan="2">
